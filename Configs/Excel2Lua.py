...
def selectInPutPath():
    # logging.info('do selectInPutPath')
    select_dir_path = askdirectory()
    path_exportExcel.set(select_dir_path)
    input_path = select_dir_path
    # logging.info('selectInPutPath is {0}'.format(input_path))
...
def iterator_files(path):
    print("iterator_files",path)
    file_name_list = os.listdir(path)
    all_file_map = {}
    for file_name in file_name_list:
        if file_name.find("~") == -1:
            file_path = os.path.join(path,file_name)
            file_map = convert_excel(file_path)
            for key in file_map:
                all_file_map[key]=file_map[key]
    
    return all_file_map

def convert_excel(path):
    #print("excel2lua start",path)
    ...
    '''
    table_name = path[path.rfind("\\")+1:path.rfind(".xlsx")]
    if len(tables) > 0:
        sheet = tables[0]
        types, keys, values = Excel2LuaUtility.prepare_sheet(sheet)
        lua_str = Excel2LuaUtility.convert_sheet(types, keys, values)
        file_map[table_name] = lua_str

    return file_map

def write_file(file_name, file_data, path):
    file_path = os.path.abspath(path + '/' + file_name + '.lua') 
...
if __name__ == '__main__':
    ...
    ttk.Button(root, text="选择路径", command=selectInPutPath, width=8).grid(row=0, column=2)
